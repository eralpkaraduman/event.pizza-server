...
router.get('/authorize/redirect', wrap(async (req, res) => {
...
router.get('/events', wrap(async (req, res) => {
  ...
  const events = await provider.getEvents(accessToken, req.query.lat, req.query.lon)
  const filteredEvents = events
    .filter(event => containsWordInDictionary(event.description))
